...
import { getGenerateImage, getDescribeImageStatus } from "@/AxiosApi/GenerativeApi"
...
const ImagePromptUI = () => {
  ...
  const { data: subscription } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { plan_name: "FREE" } // Hardcoded for testing
    },
  })

  const canMakePrivate = (planName?: string) => {
    const allowedPlans = ["Starter", "Professional", "Unlimited", "Basic Enterprise", "Premium Enterprise"]
    ...
    if (describeTaskStatus.status === "SUCCESS") {
      ...
      const animateText = () => {
        if (currentIndex < words.length) {
          setInputText((prev) => prev + (prev ? " " : "") + words[currentIndex])
          currentIndex++
