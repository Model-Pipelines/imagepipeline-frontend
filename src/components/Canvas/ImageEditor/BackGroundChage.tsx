...
export default function BackGroundChange() {
  ...
  const handleSubmit = useCallback(() => {
    if (!selectedImage) {
      toast({ title: "Error", description: "No image selected. Please select an image first.", variant: "destructive" });
      return;
    }
    if (!prompt) {
      toast({ title: "Error", description: "Please provide a prompt for the new background.", variant: "destructive" });
      return;
    }
    ...
    startBackgroundChange(payload, {
      onSuccess: (response) => {
        if (!response?.id) {
          toast({ title: "Error", description: "Invalid response structure: Missing task ID.", variant: "destructive" });
          return;
        }
  ...
  }, [selectedImage, prompt, backgroundImage, startBackgroundChange, toast, addTask, selectedImageId]);
      ...
      <CardContent className="space-y-6 pt-4">
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-1 space-y-2">
            ...
            {selectedImage ? (
              <img src={selectedImage.url} alt="Selected Image" className="w-full h-auto rounded-md border border-gray-200" />
            ) : (
            ...
            <div className="flex flex-col gap-4">
              {!backgroundImage ? (
                ...
                <div className="relative">
                  <img src={backgroundImage} alt="Uploaded Background" className="w-full h-auto rounded-md border border-gray-200" />
                  <button onClick={() => setBackgroundImage(null)} className="absolute top-2 right-2 p-2 bg-white rounded-full shadow-md hover:bg-gray-100">
        ...
        <div className="space-y-2">
          <Label htmlFor="prompt" className="text-black font-medium">Prompt</Label>
          <Input id="prompt" placeholder="Describe the new background" value={prompt} onChange={(e) => setPrompt(e.target.value)} />
        </div>
      </CardContent>
      <CardFooter>
        <Button onClick={handleSubmit} disabled={!selectedImage} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
          <TextShimmerWave className="text-white font-bold" duration={1}>Generate Background</TextShimmerWave>
        </Button>
      </CardFooter>
    </Card>
  );
