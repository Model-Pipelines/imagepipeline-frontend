...
const StyleChangeImage = () => {
    ...
    changeStyleImageMutation(
      ...
      },
      {
        onSuccess: (response) => {
          ...
          const scaleFactor =
            200 /
            Math.max(selectedImage!.size.width, selectedImage!.size.height);
          ...
          toast({
            title: "Processing",
            description: "Style change in progress...",
          });
        },
        onError: (error: any) => {
          toast({
  ...
  useEffect(() => {
    ...
    if (taskStatus.status === "SUCCESS" && taskStatus.output_url) {
      ...
      toast({
        title: "Error",
        description: taskStatus.error || "Failed to change style",
        variant: "destructive",
      });
      removePendingImage(pendingImageId);
      setTaskId(null);
              ...
              <Select value={style} onValueChange={setStyle}>
                ...
                <SelectContent>
                  {styleOptions.map((styleOption) => (
                    ...
                    >
                      {styleOption.toLowerCase()}{" "}
                      {/* Explicitly lowercase the display */}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
